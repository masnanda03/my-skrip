...
tabel_uid = {"134611", "911"}

-- [NO TOUCH BICES] --
-- Chemical IDs and Recipe Requirements
local MYSTERIOUS_CHEMICAL_ID = 922
local MAX_MYSTERIOUS_CHEMICAL = 249
local CHEMICAL_Y_ID = 924
local CHEMICAL_B_ID = 920
...
local CHEMICAL_NAMES = {
    [CHEMICAL_Y_ID] = "`9Chemical Y",
    [CHEMICAL_B_ID] = "`eChemical B", 
...
local function mufflogs(text)
    LogToConsole("`w[`bMysteriousChemicals`w] `9"..text)
end
...
local function processChemicals()
    local mysteriousChemicalCount = findItem(MYSTERIOUS_CHEMICAL_ID)
    
    if mysteriousChemicalCount > MAX_MYSTERIOUS_CHEMICAL and ADD_VEND then
        vendItem()
    end
    ...
    if allIngredientsAvailable then
        local chemicalIds = {CHEMICAL_Y_ID, CHEMICAL_B_ID, CHEMICAL_P_ID}
...
if match_found then
    ...
    Sleep(1000)
    say("`9Auto Make Mysterious Chemicals by `#@muffinncps")
